        ...
        <q-markup-table flat bordered>
          ...
          <tbody class="bg-blue-1">
            <tr v-for="(tr, i) in list_transaction" :key="'tr' + i">
              ...
              <td class="text-right">{{ tr.transaction_count }}</td>
              <td class="text-right">{{ toCurrency(tr.total_price_sum) }}</td>
              <!-- <td class="text-left">
...
export default {
  ...
  methods: {
    ...
    refreshTable() {
      ...
      this.$http
        .get("/transaction/laporan/" + this.select_period.month + "/" + this.select_period.year)
        .then((result) => {
